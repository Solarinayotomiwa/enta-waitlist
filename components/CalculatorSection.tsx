"use client";

import { ChangeEvent, CSSProperties, useEffect, useMemo, useRef, useState } from "react";
import { motion, useInView, useReducedMotion } from "motion/react";
import { cn } from "@/lib/cn";
import { figmaAssets } from "@/lib/figma-assets";
import {
  type AssetKey,
  type ComparisonResult,
  type MarketSnapshot,
  SAVINGS_EXAMPLE_NGN,
  calculateAssetComparison,
  formatComparisonDate,
  formatSnapshotTimestamp,
  formatUsd,
  formatUsdCompact,
} from "@/lib/market";

type AssetMeta = {
  key: AssetKey;
  label: string;
  symbol: string;
  icon: string;
  accent: string;
  quantityDigits: number;
  summary: string;
};

const assetMeta: AssetMeta[] = [
  {
    key: "ngn",
    label: "Held in naira",
    symbol: "NGN",
    icon: figmaAssets.calculatorChartNaira,
    accent: "#e6e7e7",
    quantityDigits: 0,
    summary: "", // computed from live rates — see nairaSummary()
  },
  {
    key: "usdt",
    label: "Held in USD₮",
    symbol: "USD₮",
    icon: figmaAssets.calculatorChartUsdt,
    accent: "#00e56b",
    quantityDigits: 2,
    summary:
      "A digital dollar that holds its value. Your savings, shielded from naira devaluation.",
  },
  {
    key: "btc",
    label: "Held in Bitcoin",
    symbol: "BTC",
    icon: figmaAssets.calculatorChartBtc,
    accent: "#ec8503",
    quantityDigits: 5,
    summary:
      "The hardest money ever made. 21 million forever — against a naira that only expands.",
  },
  {
    key: "xaut",
    label: "Held in Gold",
    symbol: "XAU₮",
    icon: figmaAssets.calculatorChartXaut,
    accent: "#f9bd03",
    quantityDigits: 3,
    summary:
      "Five thousand years as the ultimate store of value. Now able to be held without the custody and counterfeit risks.",
  },
];

const defaultAmount = SAVINGS_EXAMPLE_NGN;
const minAmount = 1;
const maxAmount = 1000000000;
const minVisibleBarScale = 0.08;

const marketUnavailableMessage = "Market data is temporarily unavailable. Please try again shortly.";

function formatNaira(value: number) {
  return new Intl.NumberFormat("en-NG", {
    currency: "NGN",
    maximumFractionDigits: 0,
    style: "currency",
  }).format(Math.round(value));
}

function parseAmountInput(value: string) {
  const next = Number(value.replace(/[^\d]/g, ""));
  if (!Number.isFinite(next)) return 0;
  return Math.min(Math.max(next, 0), maxAmount);
}

function formatAmountInput(value: number) {
  if (value <= 0) return "";
  return value.toLocaleString("en-NG");
}

function formatQuantity(value: number, digits: number, symbol: string) {
  if (!Number.isFinite(value)) return "";
  const formatted = new Intl.NumberFormat("en-US", {
    minimumFractionDigits: digits > 0 ? 2 : 0,
    maximumFractionDigits: digits,
  }).format(value);
  return `${formatted} ${symbol}`;
}

function nairaSummary(snapshot: MarketSnapshot) {
  const lostShare = 1 - snapshot.historical.ngnPerUsd / snapshot.current.ngnPerUsd;
  const percent = Math.round(Math.max(0, Math.min(1, lostShare)) * 100);
  return `The number never changed — but in dollar terms it buys ${percent}% less than when you saved it.`;
}

function getAmountHeightFactor(value: number) {
  const minLog = Math.log10(minAmount);
  const maxLog = Math.log10(maxAmount);
  const currentLog = Math.log10(Math.min(Math.max(value, minAmount), maxAmount));
  const magnitude = (currentLog - minLog) / (maxLog - minLog);

  return 0.78 + magnitude * 0.22;
}

const sectionReveal = {
  hidden: { opacity: 0, y: 24 },
  visible: { opacity: 1, y: 0 },
};

type ChartItem = AssetMeta & ComparisonResult & { chartScale: number };

function buildChartItems(results: ComparisonResult[], amount: number): ChartItem[] {
  if (results.length === 0) return [];

  const byKey = new Map(results.map((result) => [result.asset, result]));
  const maxResult = Math.max(...results.map((result) => result.currentUsdValue));
  const amountHeightFactor = getAmountHeightFactor(amount);

  return assetMeta.flatMap((meta) => {
    const result = byKey.get(meta.key);
    if (!result) return [];

    const relativeScale = maxResult > 0 ? result.currentUsdValue / maxResult : 0;
    const chartScale = Math.min(Math.max(relativeScale * amountHeightFactor, minVisibleBarScale), 1);

    return [{ ...meta, ...result, chartScale }];
  });
}

export function CalculatorSection() {
  const sectionRef = useRef<HTMLElement | null>(null);
  const reducedMotion = useReducedMotion();
  const isInView = useInView(sectionRef, { margin: "0px 0px -35% 0px", once: true });
  const contentVisible = Boolean(reducedMotion || isInView);
  const shouldAnimate = isInView && !reducedMotion;
  const [amount, setAmount] = useState(defaultAmount);
  const [amountInput, setAmountInput] = useState(defaultAmount.toLocaleString("en-NG"));
  const [expandedKey, setExpandedKey] = useState<AssetKey>("xaut");
  const [snapshot, setSnapshot] = useState<MarketSnapshot | null>(null);
  const [marketStatus, setMarketStatus] = useState<"loading" | "ready" | "error">("loading");

  /* One fetch per page view; every keystroke recalculates locally from the
     stored snapshot — no live requests while typing. */
  useEffect(() => {
    let cancelled = false;

    async function loadSnapshot() {
      try {
        const response = await fetch("/api/market");
        if (!response.ok) throw new Error(`Market API responded ${response.status}`);

        const data = (await response.json()) as MarketSnapshot;
        if (cancelled) return;

        setSnapshot(data);
        setMarketStatus("ready");
      } catch (error) {
        console.error("Market snapshot failed to load", error);
        if (!cancelled) setMarketStatus("error");
      }
    }

    loadSnapshot();
    return () => {
      cancelled = true;
    };
  }, []);

  const exampleItems = useMemo(
    () =>
      snapshot
        ? buildChartItems(calculateAssetComparison(SAVINGS_EXAMPLE_NGN, snapshot), SAVINGS_EXAMPLE_NGN)
        : [],
    [snapshot],
  );

  const chartItems = useMemo(
    () => (snapshot ? buildChartItems(calculateAssetComparison(amount, snapshot), amount) : []),
    [amount, snapshot],
  );

  const startDateLabel = formatComparisonDate(
    snapshot?.historical.date ?? "2021-01-01",
  );
  const timestampLabel = snapshot ? formatSnapshotTimestamp(snapshot.timestamp) : "";

  function onAmountChange(event: ChangeEvent<HTMLInputElement>) {
    const nextAmount = parseAmountInput(event.target.value);

    setAmount(nextAmount);
    setAmountInput(formatAmountInput(nextAmount));
  }

  return (
    <section
      className={cn(
        "calculator-section relative isolate overflow-hidden bg-[#0d101d] px-6 py-24 text-white sm:py-[120px] lg:px-0",
        shouldAnimate && "motion-active",
      )}
      id="calculator"
      ref={sectionRef}
    >
      <div aria-hidden="true" className="calculator-orb" />

      <div className="relative z-10 mx-auto flex w-full max-w-[1200px] flex-col gap-20 lg:gap-[200px]">
        <div className="grid gap-10 lg:grid-cols-[390px_1fr] lg:gap-12">
          <motion.div
            animate={contentVisible ? "visible" : "hidden"}
            className="max-w-[410px]"
            initial="hidden"
            transition={{ duration: 0.55, ease: "easeOut" }}
            variants={sectionReveal}
          >
            <p className="mb-3 text-xl leading-[30px] text-[#4b8bff]">The savings problem</p>
            <h2 className="text-balance text-[2.35rem] font-semibold leading-[1.12] tracking-[-0.018em] sm:text-5xl sm:leading-[54px]">
              You saved {formatNaira(SAVINGS_EXAMPLE_NGN)}. But did you really?
            </h2>
            <p className="mt-4 text-pretty text-lg leading-[26px] tracking-[-0.32px] text-white sm:text-xl">
              If you had held {formatNaira(SAVINGS_EXAMPLE_NGN)} in each option since{" "}
              {startDateLabel}, this is what it could be worth today — in US dollars.
            </p>
            <a
              className="mt-10 inline-flex h-10 items-center justify-center rounded-lg bg-white px-3.5 text-sm font-semibold capitalize text-[#0c111d] transition duration-150 ease-out hover:bg-white/90"
              href="#waitlist-form"
            >
              Join our waitlist
            </a>
          </motion.div>

          <motion.div
            animate={contentVisible ? "visible" : "hidden"}
            className="grid gap-8 lg:grid-cols-[387px_1fr]"
            initial="hidden"
            transition={{ delayChildren: 0.1, staggerChildren: 0.08 }}
          >
            <motion.div
              className="relative min-h-[440px] overflow-hidden rounded-2xl border border-[#414167] bg-[#111827] lg:min-h-[541px]"
              transition={{ duration: 0.5, ease: "easeOut" }}
              variants={sectionReveal}
            >
              <img
                alt=""
                className="absolute inset-0 size-full object-cover opacity-70"
                src={figmaAssets.calculatorBg}
              />
              <img
                alt=""
                className="absolute inset-0 size-full object-cover"
                src={figmaAssets.calculatorHandMoneyMulti}
              />
            </motion.div>

            <motion.div className="flex flex-col justify-center gap-6" variants={sectionReveal}>
              {marketStatus === "error" ? (
                <p className="text-pretty text-xl leading-[27px] text-white/70">
                  {marketUnavailableMessage}
                </p>
              ) : (
                exampleItems.map((item) => {
                  const rows = exampleItems.filter((entry) => entry.key !== expandedKey);
                  const isLastRow = rows[rows.length - 1]?.key === item.key;

                  if (item.key === expandedKey) {
                    return (
                      <motion.div
                        className="min-h-[324px] overflow-hidden rounded-2xl border border-[#2a2a49] px-5 py-6"
                        key={item.key}
                        layout
                        transition={{ layout: { duration: 0.38, ease: [0.32, 0.72, 0, 1] } }}
                      >
                        <motion.div
                          animate={{ opacity: 1, y: 0 }}
                          initial={{ opacity: 0, y: 8 }}
                          transition={{ delay: 0.05, duration: 0.3, ease: "easeOut" }}
                        >
                          <img
                            alt=""
                            className="h-[66px] w-auto object-contain drop-shadow-[0_4px_4px_rgba(0,0,0,0.25)]"
                            src={item.icon}
                          />
                          <p className="mt-3 text-[2rem] font-medium leading-none tracking-[-0.576px] tabular-nums text-white">
                            {formatUsd(item.currentUsdValue)}
                          </p>
                          {item.key !== "ngn" ? (
                            <p className="mt-2 text-sm leading-5 text-white/50">
                              {formatQuantity(item.historicalQuantity, item.quantityDigits, item.symbol)}{" "}
                              bought on {startDateLabel}
                            </p>
                          ) : null}
                          <p className="mt-5 text-base font-medium leading-[23px] tracking-[-0.32px] text-white/60">
                            {item.label}
                          </p>
                          <p className="mt-3 text-pretty text-xl leading-[26px] tracking-[-0.32px] text-white">
                            {item.key === "ngn" && snapshot ? nairaSummary(snapshot) : item.summary}
                          </p>
                        </motion.div>
                      </motion.div>
                    );
                  }

                  return (
                    <motion.button
                      className={cn(
                        "flex w-full cursor-pointer items-start justify-between gap-6 pb-3 text-left",
                        !isLastRow && "border-b border-[#414167]",
                      )}
                      key={item.key}
                      layout
                      onClick={() => setExpandedKey(item.key)}
                      onFocus={() => setExpandedKey(item.key)}
                      onMouseEnter={() => setExpandedKey(item.key)}
                      transition={{ layout: { duration: 0.38, ease: [0.32, 0.72, 0, 1] } }}
                      type="button"
                    >
                      <motion.span
                        animate={{ opacity: 1 }}
                        className="flex w-full items-start justify-between gap-6"
                        initial={{ opacity: 0 }}
                        transition={{ delay: 0.04, duration: 0.25, ease: "easeOut" }}
                      >
                        <p className="text-base font-medium leading-[23px] tracking-[-0.32px] text-white/60">
                          {item.label}
                        </p>
                        <p className="text-2xl font-medium tracking-[-0.432px] tabular-nums text-white">
                          {formatUsd(item.currentUsdValue)}
                        </p>
                      </motion.span>
                    </motion.button>
                  );
                })
              )}
              {marketStatus === "loading" ? (
                <p className="text-base leading-6 text-white/50">Loading current market rates…</p>
              ) : null}
            </motion.div>
          </motion.div>
        </div>

        <motion.div
          animate={contentVisible ? "visible" : "hidden"}
          className="overflow-hidden rounded-2xl border-[3px] border-[#eeaafd]"
          initial="hidden"
          transition={{ duration: 0.55, ease: "easeOut" }}
          variants={sectionReveal}
        >
          <div className="grid gap-10 p-6 sm:p-10 lg:grid-cols-[397px_1fr] lg:items-center lg:gap-[50px] lg:py-14 lg:pl-16 lg:pr-[21px]">
            <div className="flex flex-col justify-center">
              <h3 className="text-balance text-[2.35rem] font-semibold leading-[1.08] tracking-[-0.018em] sm:text-5xl sm:leading-[52px]">
                What would your number look like?
              </h3>
              <label className="mt-6 block">
                <span
                  className="flex items-center rounded-lg border border-white bg-black/20 py-4 pl-[23px] pr-5 sm:py-5"
                  data-calculator-amount-field
                >
                  <span className="mr-1 text-4xl font-semibold leading-none sm:text-[48px] sm:leading-[60px] sm:tracking-[-0.96px]">
                    ₦
                  </span>
                  <input
                    aria-label="Amount saved in naira"
                    className="w-full bg-transparent text-4xl font-semibold leading-none tracking-[-0.96px] text-white outline-none placeholder:text-white/30 sm:text-[48px] sm:leading-[60px]"
                    data-calculator-amount-input
                    inputMode="numeric"
                    onChange={onAmountChange}
                    value={amountInput}
                  />
                </span>
              </label>
              <p className="mt-4 text-pretty text-xl leading-[27px] tracking-[-0.32px] text-white/80">
                Enter an amount in naira to see what it could be worth today, in US dollars, if it
                had been held in each asset since {startDateLabel}.
              </p>
            </div>

            <div className="flex flex-col">
              <div className="relative min-h-[430px] overflow-hidden rounded-[11px] bg-[#111a3a] lg:h-[430px]">
                {marketStatus === "error" ? (
                  <div className="absolute inset-0 flex items-center justify-center p-8">
                    <p className="max-w-[360px] text-center text-lg leading-6 text-white/70">
                      {marketUnavailableMessage}
                    </p>
                  </div>
                ) : (
                  <div className="absolute inset-x-5 top-14 bottom-6 grid grid-cols-4 gap-3 sm:inset-x-[34px] sm:top-[81px] sm:bottom-[30px] sm:gap-[18px]">
                    {chartItems.map((item, index) => (
                      <div className="flex h-full flex-col" key={item.key}>
                        <div
                          className="relative flex flex-1 items-end justify-center"
                          data-chart-scale={item.chartScale.toFixed(4)}
                        >
                          {index > 0 ? (
                            <img
                              alt=""
                              aria-hidden="true"
                              className="absolute -left-[7px] top-0 h-full w-px sm:-left-[10px]"
                              src={figmaAssets.calculatorChartDivider}
                            />
                          ) : null}
                          <div
                            className="absolute left-1/2 z-20 flex -translate-x-1/2 flex-col items-center gap-1 text-center transition-[bottom] duration-[550ms] ease-out"
                            style={{ bottom: `calc(${(item.chartScale * 100).toFixed(3)}% + 8px)` }}
                          >
                            <p className="whitespace-nowrap text-[10.6px] font-normal leading-4 text-white">
                              {item.label}
                            </p>
                            <img alt="" className="size-7" src={item.icon} />
                          </div>
                          <div
                            aria-label={`${item.label}: ${formatUsd(item.currentUsdValue)}`}
                            className="comparison-bar relative w-full max-w-[123px] cursor-pointer overflow-hidden rounded-[11px] transition-[height,filter,transform] duration-[550ms] ease-out hover:scale-[1.03] hover:brightness-[1.12]"
                            style={
                              {
                                background: item.accent,
                                height: `${(item.chartScale * 100).toFixed(3)}%`,
                                transformOrigin: "bottom",
                              } as CSSProperties
                            }
                          />
                        </div>
                        <p className="comparison-bar-value mt-3 shrink-0 text-center text-[11px] font-medium tracking-[-0.2px] tabular-nums text-white sm:text-[15px]">
                          <span className="sm:hidden">{formatUsdCompact(item.currentUsdValue)}</span>
                          <span className="hidden sm:inline">{formatUsd(item.currentUsdValue)}</span>
                        </p>
                      </div>
                    ))}
                  </div>
                )}
              </div>

              <div className="mt-4 flex flex-col gap-1.5 px-1 text-[13px] leading-5 text-white/55">
                {snapshot ? (
                  <p className="rate-source">
                    {snapshot.stale ? "Last updated" : "Rates as of"} {timestampLabel}. Source:{" "}
                    <a
                      className="underline decoration-white/30 underline-offset-2 transition hover:text-white/80"
                      href={snapshot.sources.assets.url}
                      rel="noopener noreferrer"
                      target="_blank"
                    >
                      {snapshot.sources.assets.name}
                    </a>
                    . Historical prices based on{" "}
                    <a
                      className="underline decoration-white/30 underline-offset-2 transition hover:text-white/80"
                      href={snapshot.sources.historical.url}
                      rel="noopener noreferrer"
                      target="_blank"
                    >
                      {snapshot.sources.historical.name}
                    </a>{" "}
                    market data from {startDateLabel}.
                  </p>
                ) : null}
              </div>
            </div>
          </div>
        </motion.div>
      </div>
    </section>
  );
}
