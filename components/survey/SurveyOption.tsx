...
export function SurveyOption({
  ...
  return (
    <button
      ...
      className={cn(
        "flex items-center gap-[13px] rounded-[14px] border bg-white px-[17px] py-[15px] text-left text-[15.5px] font-medium leading-[1.35] text-[#0B2036] shadow-[0_1px_2px_rgba(11,32,54,.04),0_8px_28px_-12px_rgba(11,32,54,.14)] outline-none transition duration-150 hover:-translate-y-0.5 hover:border-[color:var(--survey-accent)] focus-visible:outline-2 focus-visible:outline-offset-[3px] focus-visible:outline-[color:var(--survey-accent)] motion-reduce:transition-none motion-reduce:hover:translate-y-0",
        selected
    ...
    >
      <span
        ...
        className={cn(
          ...
          selected
            ...
            : "bg-[rgba(11,32,54,.05)] text-[#7A93A5]",
        )}
      >
        {index + 1}
      </span>
      ...
      {multi ? (
        <span aria-hidden="true" className="ml-auto text-sm font-semibold text-[color:var(--survey-accent)]">
          {selected ? "✓" : ""}
        </span>
      ) : null}
    </button>
